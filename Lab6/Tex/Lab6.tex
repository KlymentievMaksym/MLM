...
\begin{document}
    ...
    \section{Демонстрація роботи}
        Код було запущено у хмарному середовищі Google Colab з використанням офіційного репозиторію авторів. Було завантажено попередньо навчені ваги.

        % \begin{figure}[h!]
        %     \centering
        %     % Переконайся, що файл result.png лежить поруч із .tex файлом!
        %     \includegraphics[width=0.9\textwidth]{../images/result.png} 
        %     \caption{Результат роботи алгоритму: зліва --- вхідне ч/б зображення, справа --- результат автоматичної колоризації.}
        %     \label{fig:result}
        % \end{figure}

        % На рис. \ref{fig:result} видно, що модель успішно відновила кольори, правильно ідентифікувавши небо (синє), траву (зелена) та об'єкт на передньому плані.
